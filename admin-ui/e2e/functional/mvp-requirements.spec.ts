...
test.describe('MVP Functional Requirements', () => {
  test.describe('Authentication & Authorization', () => {
    ...
    test('login fails with invalid credentials', async ({ page }) => {
      ...
      await expect(page.getByText(/Invalid credentials|Неверные учетные данные/i)).toBeVisible({
        timeout: 5000,
      });
    });
  });
  ...
  test.describe('POS Operations', () => {
    ...
    test('add product to cart from catalog', async ({ page, request }) => {
       ...
       * This test creates a product via API using static token in x-admin-secret header
       * Line 171: headers: { 'x-admin-secret': token }
       */
      ...
      await select.selectOption({ label: new RegExp(name).source });
  ...
  test.describe('Product Management', () => {
    ...
    test('filter products by type', async ({ page, request }) => {
       ...
       * ⚠️ LEGACY AUTH PATTERN: Uses x-admin-secret header instead of JWT cookies
       * This test creates a product via API using static token in x-admin-secret header
       * Line 231: headers: { 'x-admin-secret': token }
       */
