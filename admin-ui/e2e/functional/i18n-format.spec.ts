...
async function switchLanguage(page: any, langCode: string) {
...
test.describe('Language Switching Tests', () => {
  ...
  test('should switch between RU/EN/SRB locales in the UI', async ({ page }) => {
    ...
    let langButton = page.locator('.language-switcher-button')
  ...
  test('should apply locale configs to the UI', async ({ page }) => {
    ...
    for (const locale of locales.supported_locales) {
      ...
      await expect(page.locator('.language-switcher-button')).toContainText(locale.code.toUpperCase())
...
test.describe('UI Logic - Sale to Client Navigation', () => {
  ...
  test('should navigate from dashboard summary to client card', async ({ page }) => {
    ...
    await page.route('**/api/v1/dashboard', async route => {
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          avg_check: 600,
          recent_sales: [
            { id: 1, client_id: 100, client_name: 'Test Client 1', total: 500, created_at: new Date().toISOString() },
            { id: 2, client_id: 101, client_name: 'Test Client 2', total: 750, created_at: new Date().toISOString() }
          ]
        })
      })
    })
    ...
    if (await recentSalesSection.isVisible()) {
      ...
      if (await clientLink.isVisible()) {
        ...
        await expect(page.locator('text=/Клиент|Client/i').first()).toBeVisible({ timeout: 5000 }).catch(() => {
  ...
  test('should verify client card displays correct information', async ({ page }) => {
    ...
    if (await clientRow.isVisible()) {
      ...
      await expect(page.locator('text=/Телефон|Phone/i').first()).toBeVisible({ timeout: 5000 }).catch(() => {
...
test.describe('Mobile Responsiveness Tests', () => {
  ...
  test('should view products on mobile viewport', async ({ page }) => {
    ...
    if (await productsLink.isVisible()) {
      ...
      await expect(page.getByText('Товары / услуги')).toBeVisible({ timeout: 5000 }).catch(() => {
  ...
  test('should collapse navigation menu on mobile', async ({ page }) => {
    ...
    const menuButton = page.locator('button[class*="menu"], button[class*="Menu"], [class*="hamburger"]').first()
...
test.describe('Edge Cases and Error Handling', () => {
  ...
  test('should verify fixture test scenarios are valid', () => {
    ...
    const defaultScenario = locales.test_scenarios.find((s: any) => s.scenario === 'Default locale')
    expect(defaultScenario.expected_code).toBe('ru')
    
    const queryScenario = locales.test_scenarios.find((s: any) => s.scenario === 'Query parameter override')
