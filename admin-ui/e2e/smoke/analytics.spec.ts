...
test('analytics recent sales links to customer profile', async ({ page }) => {
  ...
  // Extract customer name from the first sale before clicking
  const customerNameElement = firstItem.locator('div').filter({ hasText: /^[А-Яа-яёЁA-Za-z]/ }).nth(2)
  const customerName = await customerNameElement.textContent()
  console.log(`[Test] Found last sale for customer: ${customerName}`)
  
  // Click on the customer name link to navigate to customer profile
  // The customer name is clickable (has onClick that navigates to customer profile)
  const customerLink = firstItem.locator('div').filter({ hasText: /^[А-Яа-яёЁA-Za-z]/ }).nth(2)
...
test('analytics dashboard shows summary metrics', async ({ page }) => {
  ...
  await expect(page.locator('.sales-trend, .chart').first()).toBeVisible({ timeout: 5000 }).catch(() => {
