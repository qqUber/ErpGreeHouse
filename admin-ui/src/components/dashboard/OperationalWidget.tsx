  ...
  return (
    ...
    >
      <div className="row mb-4">
        <div className="operational-widget-title">Оперативные данные</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="operational-widget">
          <div className="grid grid-cols-2 gap-4">
            <div className="card cardCompact">
              <div className="operational-metric-label">Активный персонал</div>
              <div className="operational-metric-value">{data.active_staff}</div>
            </div>
            <div className="card cardCompact">
              <div className="operational-metric-label">Всего транз.</div>
              <div className="operational-metric-value">{data.total_transactions}</div>
            </div>
            <div className="card cardCompact">
              <div className="operational-metric-label">{t('analytics.revenue')}</div>
              <div className="operational-metric-value">
                {(data.total_revenue || 0).toFixed(2)} ₽
              </div>
            </div>
            <div className="card cardCompact">
              <div className="operational-metric-label">{t('analytics.averageCheck')}</div>
              <div className="operational-metric-value">
          ...
          <div className="card cardCompact operational-peak-card">
            <div className="operational-peak-label">Пиковый час</div>
        ...
        {/* Top Products */}
        <div>
          <div className="operational-section-title">Топ товаров</div>
          <div className="operational-widget">
            {(data.top_products || []).slice(0, 5).map((product: any) => (
              <div key={product.code} className="row operational-product-row">
                <div className="flex-1">
                  <div className="operational-product-name">{product.name}</div>
                  <div className="operational-product-code">{product.code}</div>
                </div>
                <div className="text-right">
                  <div className="operational-product-qty">{product.quantity} шт</div>
      ...
      {(data.hourly_breakdown || []).length > 0 && (
        <div className="operational-hourly-section">
          <div className="operational-section-title">{t('analytics.salesByHour')}</div>
            ...
            <div className="grid grid-cols-12 gap-2 operational-hourly-grid">
              {data.hourly_breakdown.map((hourData: any) => (
                <div key={hourData.hour} className="text-center">
                  <div className="operational-hour-label">{hourData.hour}:00</div>
                  <div className="operational-hour-value">{hourData.transactions}</div>
                </div>
              ))}
            </div>
          </div>
