...
import { ImportResult, ImportPreview } from '../api'
...
export function ProductImport({ onImportComplete, onClose }: ProductImportProps) {
  ...
  const handleImportFile = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/v1/products/import/file', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('admin_token') || localStorage.getItem('token')}`
  ...
  const handleImportUrl = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const err = await response.json()
        throw new Error(err.detail || 'Failed to import from URL')
      }
          ...
          {result && (
            <div style={{ 
              ...
              <div style={{ display: 'flex', gap: 16 }}>
                ...
                <div style={{ color: '#16a34a' }}>Создано: <strong>{result.created}</strong></div>
                <div style={{ color: '#2563eb' }}>Обновлено: <strong>{result.updated}</strong></div>
                {result.errors.length > 0 && (
                  <div style={{ color: '#dc2626' }}>Ошибок: <strong>{result.errors.length}</strong></div>
                )}
              </div>
