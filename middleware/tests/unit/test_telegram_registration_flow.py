...
class TestStartCommand:
    ...
    def test_start_new_user_shows_welcome_and_consent_buttons(self, setup_test_db, clean_redis, sample_user):
        ...
        with patch('app.handlers.ERPClient') as MockERPClient:
            ...
            with patch('app.handlers.get_redis') as mock_redis:
                ...
                # Check reply_markup has consent buttons
                assert call_args[1].get('reply_markup') is not None
                keyboard = call_args[1]['reply_markup'].inline_keyboard
                
                # Verify consent buttons exist
                button_texts = [btn.text for row in keyboard for btn in row]
                assert any("Пользовательское соглашение" in text for text in button_texts)
                assert any("152-ФЗ" in text for text in button_texts)
                assert any("Отказ" in text for text in button_texts)
    ...
    def test_start_existing_user_shows_balance_and_consent_status(self, setup_test_db, clean_redis, sample_user):
        ...
        from app.handlers import _upsert_local_customer
        ...
        with patch('app.handlers.ERPClient') as MockERPClient:
            mock_client = Mock()
            mock_client.get_customer_by_telegram_id = AsyncMock(return_value={
                "name": "Test User",
                "telegram_id": sample_user["id"]
            })
...
class TestConsentCallback:
    ...
    def test_consent_agree_proceeds_to_name_request(self, setup_test_db, clean_redis, sample_user):
    ...
    def test_consent_refuse_cleans_user_data(self, setup_test_db, clean_redis, sample_user):
        ...
        from app.handlers import cb_consent, _cleanup_user_data
        ...
        with patch('app.handlers.get_redis') as mock_redis:
            ...
            assert "отказались" in call_args[0][0].lower() or "удалены" in call_args[0][0].lower()
...
class TestRegistrationMessage:
    ...
    def test_name_input_stores_name_and_asks_for_phone(self, setup_test_db, clean_redis, sample_user):
        ...
        with patch('app.handlers.get_redis') as mock_redis:
            ...
            call_args = message.answer.call_args
            assert "Телефон" in call_args[0][0]
    
    def test_phone_input_stores_phone_and_asks_for_marketing(self, setup_test_db, clean_redis, sample_user):
    ...
    def test_marketing_yes_creates_customer_with_marketing(self, setup_test_db, clean_redis, sample_user):
        ...
        with patch('app.handlers.get_redis') as mock_redis:
            ...
            with patch('app.handlers.ERPClient') as MockERPClient:
                ...
                cb = MockCallbackQuery(sample_user, "consent:marketing:yes")
    ...
    def test_marketing_no_creates_customer_without_marketing(self, setup_test_db, clean_redis, sample_user):
        ...
        with patch('app.handlers.get_redis') as mock_redis:
            ...
            with patch('app.handlers.ERPClient') as MockERPClient:
                ...
                cb = MockCallbackQuery(sample_user, "consent:marketing:no")
...
class TestCleanupUserData:
    ...
    def test_cleanup_removes_customer_and_consents(self, setup_test_db, sample_user):
        ...
        from app.handlers import _cleanup_user_data, _store_consent, _upsert_local_customer
        ...
        with patch('app.handlers.get_redis') as mock_redis:
            ...
            assert mock_r.delete.call_count == 3  # cart, consent, registration keys
...
class TestRegistrationFlow:
    ...
    def test_full_registration_flow_with_marketing(self, setup_test_db, clean_redis, sample_user):
        """Test complete registration flow: start → consent → name → phone → marketing yes."""
        from app.handlers import cmd_start, cb_consent, handle_registration_message, cb_marketing_consent
    ...
    def test_registration_refusal_cleans_all_data(self, setup_test_db, clean_redis, sample_user):
