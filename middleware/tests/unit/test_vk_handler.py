...
class TestVKKeyboard:
    ...
    def test_format_vk_keyboard_structure(self, setup_test_db):
        ...
        from app.integrations.bots.shared.keyboards import format_vk_keyboard, get_consent_buttons
...
class TestUpdateConsent:
    ...
    def test_update_consent_marketing_only_tg(self, setup_test_db):
        ...
        from app.integrations.bots.shared.consent import update_consent, get_customer_consents
        from app.db import get_db
...
class TestCleanupUserData:
    ...
    def test_cleanup_user_data_tg(self, setup_test_db, clean_redis):
        """Test cleaning up user data for Telegram user."""
        from app.integrations.bots.shared.consent import cleanup_user_data, get_customer_consents
        from app.db import get_db
        ...
        with patch('app.integrations.bots.shared.consent.get_redis') as mock_redis:
            ...
            # Cleanup user data
            cleanup_user_data("tg", 999001)
        
        # Verify user is deleted
        consents = get_customer_consents("tg", 999001)
        assert consents["marketing_allowed"] is False
        assert consents["data_processing_allowed"] is False
    
    def test_cleanup_user_data_tg_another(self, setup_test_db, clean_redis):
        """Test cleaning up another Telegram user."""
        from app.integrations.bots.shared.consent import cleanup_user_data, get_customer_consents
        from app.db import get_db
        ...
        with patch('app.integrations.bots.shared.consent.get_redis') as mock_redis:
            ...
            # Cleanup
            cleanup_user_data("tg", 999002)
        
        # Verify deleted
        consents = get_customer_consents("tg", 999002)
        assert consents["marketing_allowed"] is False
        assert consents["data_processing_allowed"] is False
    ...
    def test_cleanup_user_data_creates_refusal_record(self, setup_test_db, clean_redis):
        ...
        conn2 = db2.connect()
        cur = conn2.execute(
            "SELECT consent_text FROM consents WHERE customer_id = ? AND consent_type = ?",
            (customer_id, "data_processing")
        )
        consent = cur.fetchone()
        assert consent is not None
    ...
    def test_complete_registration_new_user_vk(self, setup_test_db, clean_redis):
        ...
        with patch('app.integrations.bots.shared.registration.get_redis'):
            with patch('app.integrations.bots.shared.registration.get_db') as mock_db:
                with patch('app.integrations.bots.shared.registration.store_consent') as mock_store:
                    # Setup mock database
                    mock_conn = MagicMock()
        ...
        with patch('app.integrations.bots.shared.registration.get_redis'):
            with patch('app.integrations.bots.shared.registration.get_db') as mock_db:
                with patch('app.integrations.bots.shared.registration.store_consent'):
                    ...
                    mock_cursor.fetchone.side_effect = [existing_customer, existing_customer]
                    mock_conn.execute.return_value = mock_cursor
                    mock_db.return_value.connect.return_value = mock_conn
