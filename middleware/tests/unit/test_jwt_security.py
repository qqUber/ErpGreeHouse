...
class TestJWTPayloadInjection:
    ...
    def test_role_escalation_prevention(self):
        ...
        settings = get_settings()
        try:
            escalated_token = jwt.encode(escalated_payload, settings.jwt_secret_key, algorithm=settings.jwt_algorithm)
...
class TestJWTCryptographicStrength:
    ...
    def test_secret_key_strength(self):
        ...
        assert len(settings.jwt_secret_key) >= 32, "Secret key should be at least 32 characters"
        ...
        assert unique_chars >= len(settings.jwt_secret_key) * 0.5, "Secret key should have good character diversity"
    ...
    def test_signature_uniqueness(self):
        ...
        for payload in payloads:
            ...
            token = jwt.encode(payload_with_meta, settings.jwt_secret_key, algorithm=settings.jwt_algorithm)
