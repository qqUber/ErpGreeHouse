...
class QRTokenFunctionalTest:
    ...
    def test_concurrent_simulation(self, test_db):
        ...
        for process_id in range(5):
            ...
            for i in range(20):
                ...
                test_db.execute("INSERT INTO customers (phone) VALUES (?)", (f"proc{process_id}_user{i}",))
                token = generate_unique_qr_token(test_db)
                tokens.append(token)
    ...
    def test_error_recovery_functional(self, test_db):
        ...
        from app.customer_identity import resolve_or_create_customer, CustomerIdentityConflictError
