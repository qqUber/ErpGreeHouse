...
async def test_erp_client_get_customer_by_phone(mock_erp_client, test_customer_data):
    ...
    mock_erp_client.get_customer_by_phone.assert_called_once_with(test_customer_data['phone'])
...
async def test_erp_client_create_customer(mock_erp_client, test_customer_data):
    ...
    # Setup mock response
    mock_erp_client.create_customer = AsyncMock(return_value={
...
async def test_erp_client_add_loyalty_points(mock_erp_client):
    ...
    # Setup mock response
    mock_erp_client.add_loyalty_points = AsyncMock(return_value={
        'success': True,
...
async def test_erp_client_redeem_loyalty_points(mock_erp_client):
    ...
    # Setup mock response
    mock_erp_client.redeem_loyalty_points = AsyncMock(return_value={
        'success': True,
...
async def test_erp_client_create_order(mock_erp_client, test_order_data):
    """Test creating order"""
    # Setup mock response
    mock_erp_client.create_order = AsyncMock(return_value={
        'order_id': 'ORD-00001',
        'success': True,
...
async def test_erp_client_get_customer_orders(mock_erp_client):
    ...
    # Setup mock response
    mock_erp_client.get_customer_orders = AsyncMock(return_value=[
        {
            'order_id': 'ORD-00001',
            'status': 'completed',
            'total': 1000.0,
...
@pytest.mark.skip(reason="Retry logic test requires actual ERP client implementation with retry")
@pytest.mark.asyncio
