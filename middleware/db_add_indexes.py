...
INDEXES_TO_ADD: List[Tuple[str, str, str]] = [
    ...
    (
        "idx_marketing_trigger_events_trigger_id",
        "marketing_trigger_events",
        "trigger_id",
    ),
    (
        "idx_marketing_trigger_events_customer_id",
        "marketing_trigger_events",
        "customer_id",
    ),
    (
        "idx_marketing_trigger_events_created_at",
        "marketing_trigger_events",
        "created_at",
    ),
...
class IndexMigration:
    ...
    def create_index(
        ...
        if not self.column_exists(table_name, column_name):
            print(
                f"⚠ Skipping index {index_name}: column {column_name} does not exist in table {table_name}"
            )
            return False
    ...
    def apply_indexes(self, dry_run: bool = False) -> Tuple[int, int, int]:
        ...
        for index_name, table_name, column_name in INDEXES_TO_ADD:
            ...
            if not self.column_exists(table_name, column_name):
                print(
                    f"[WARN] Skipping {index_name}: column {column_name} does not exist in {table_name}"
                )
                skipped_no_column += 1
                continue
            ...
            if dry_run:
                print(
                    f"[WOULD] Create index: {index_name} on {table_name}({column_name})"
                )
                applied += 1
            else:
