...
class LoyaltyRules:
    ...
    tiers: List[Tier] = field(default_factory=lambda: [
        ...
        Tier(name="Серебро", min_spent=10000, accrual_percent=7, max_redeem_percent=50),
        Tier(name="Золото", min_spent=50000, accrual_percent=10, max_redeem_percent=100),
        Tier(name="Платина", min_spent=100000, accrual_percent=15, max_redeem_percent=100),
    ])
...
def clamp_redeem_points(total_amount: int, spent_amount: int, requested_points: int, available_points: int, rules: LoyaltyRules) -> int:
