...
COLUMN_ALIASES = {
    'name': ['name', 'название', 'title', 'product_name', 'productname', 'товар', 'наименование'],
...
def parse_xml(content: bytes) -> list[dict]:
    ...
    product_paths = [
        './/product', './/item', './/good', './/товар', './/products', './/items',
        './/catalog/product', './/catalog/item', './/data/product'
    ]
...
def validate_product(row: dict, existing_skus: set) -> tuple[dict | None, str | None]:
    ...
    if sku in existing_skus:
        return None, f"Row {row.get('_row', '?')}: SKU '{sku}' already exists"
...
def import_products_url(
    ...
    try:
        if format_lower == "json":
            ...
            raise HTTPException(status_code=400, detail=f"Unsupported format: {payload.format}. Use 'json' or 'xml'")
    except HTTPException:
        raise
    except Exception as e:
...
def process_import(rows: list[dict], import_type: str) -> dict[str, Any]:
    ...
    try:
        # Get existing SKUs
        cur = conn.execute("SELECT code FROM products")
        existing_skus = {str(r["code"]).strip() for r in cur.fetchall()}
