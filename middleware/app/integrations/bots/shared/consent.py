...
def cleanup_user_data(source: Source, user_id: int, conn=None) -> None:
    """
    Clean all client data when user refuses registration (152-ФЗ compliance).
    ...
    try:
        ...
        cur = conn.execute(
            f"SELECT id FROM customers WHERE {id_column} = ?",
            (user_id,)
        )
        row = cur.fetchone()
        customer_id = row["id"] if row else None
        
        # Log the refusal for compliance
        if customer_id:
            store_consent(
                customer_id,
                source,
                ...
                "data_processing",
                conn  # Reuse connection here too
            )
            # Delete consents
