...
**Core principle:** KiloCode automates everything with CLI/API. Checkpoints are for verification and decisions, not manual work.

**Golden rules:**

1. **If KiloCode can run it, KiloCode runs it** - Never ask user to execute CLI commands, start servers, or run builds
2. **KiloCode sets up the verification environment** - Start dev servers, seed databases, configure env vars
3. **User only does what requires human judgment** - Visual checks, UX evaluation, "does this feel right?"
4. **Secrets come from user, automation comes from KiloCode** - Ask for API keys, then KiloCode uses them via CLI
   </overview>
...
**When:** KiloCode completed automated work, human confirms it works correctly.
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>[What KiloCode automated and deployed/built]</what-built>
  <how-to-verify>
...
- `<what-built>`: What KiloCode automated (deployed, built, configured)
...
**Key pattern:** KiloCode starts the dev server BEFORE the checkpoint. User only needs to visit the URL.
...
**When:** Action has NO CLI/API and requires human-only interaction, OR KiloCode hit an authentication gate during automation.

**Use ONLY for:**

- **Authentication gates** - KiloCode tried to use CLI/API but needs credentials to continue (this is NOT a failure)
...
<task type="checkpoint:human-action" gate="blocking">
  <action>[What human must do - KiloCode already did everything automatable]</action>
  <instructions>
    [What KiloCode already automated]
    [The ONE thing requiring human action]
  </instructions>
  <verification>[What KiloCode can check afterward]</verification>
  <resume-signal>[How to continue]</resume-signal>
</task>
```

**Key principle:** KiloCode automates EVERYTHING possible first, only asks human for the truly unavoidable manual step.
...
<!-- If vercel returns "Error: Not authenticated", KiloCode creates checkpoint on the fly -->
...
<!-- After authentication, KiloCode retries the deployment -->
...
**Key distinction:** Authentication gates are created dynamically when KiloCode encounters auth errors during automation. They're NOT pre-planned - KiloCode tries to automate first, only asks for credentials when blocked.
</type>
</checkpoint_types>

<execution_protocol>

When KiloCode encounters `type="checkpoint:*"`:
...
**Critical:** When KiloCode tries CLI/API and gets auth error, this is NOT a failure - it's a gate requiring human input to unblock automation.

**Pattern:** KiloCode tries automation → auth error → creates checkpoint → you authenticate → KiloCode retries → continues
...
```
KiloCode: Running `vercel --yes` to deploy...
...
KiloCode: Verifying authentication...
...
- Pre-planned checkpoint: "I need you to do X" (wrong - KiloCode should automate)
...
**The rule:** If it has CLI/API, KiloCode does it. Never ask human to perform automatable work.
...
<!-- RIGHT: KiloCode asks for value, then adds via CLI -->
<task type="checkpoint:human-action">
...
**KiloCode starts servers, user visits URLs:**
...
```xml
<!-- KiloCode starts server before checkpoint -->
<task type="auto">
...
| Action                           | Automatable?               | KiloCode does it? |
| -------------------------------- | -------------------------- | ----------------- |
...
- Ask human to do work KiloCode can automate (deploy, create resources, run builds)
- Assume knowledge: "Configure the usual settings" ❌
- Skip steps: "Set up database" ❌ (too vague)
- Mix multiple verifications in one checkpoint (split them)
- Make verification impossible (KiloCode can't check visual appearance without user confirmation)

**Placement:**

- **After automation completes** - not before KiloCode does the work
...
- Before KiloCode automates (asking human to do automatable work) ❌
...
<!-- NO CHECKPOINT NEEDED - KiloCode automated everything and verified programmatically -->
```
...
<!-- ONE checkpoint at end verifies the complete flow - KiloCode already started server -->
<task type="checkpoint:human-verify" gate="blocking">
...
**Why bad:** KiloCode can run `npm run dev`. User should only visit URLs, not execute commands.

### ✅ GOOD: KiloCode starts server, user visits
...
**Why bad:** Convex has `npx convex env set`. KiloCode should ask for the key value, then run the CLI command.

### ✅ GOOD: KiloCode collects secret, adds via CLI
...
**Why bad:** Vercel has a CLI. KiloCode should run `vercel --yes`.

### ✅ GOOD: KiloCode automates, human verifies
...
**Why bad:** KiloCode has Write tool. This should be `type="auto"`.
...
**Why bad:** KiloCode can run these commands. User should never execute CLI commands.
...
**Why bad:** Stripe has an API. KiloCode should create the webhook via API and write to .env directly.

</anti_patterns>

<summary>

Checkpoints formalize human-in-the-loop points. Use them when KiloCode cannot complete a task autonomously OR when human verification is required for correctness.

**The golden rule:** If KiloCode CAN automate it, KiloCode MUST automate it.

**Checkpoint priority:**

1. **checkpoint:human-verify** (90% of checkpoints) - KiloCode automated everything, human confirms visual/functional correctness
...
- Things KiloCode can verify programmatically (tests pass, build succeeds)
- File operations (KiloCode can read files to verify)
