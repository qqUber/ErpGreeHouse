# ErpGreeHouse - State

## Project Reference
**Core Value:** A centralized loyalty and communications platform for coffee shops that saves 50-100k ₽/month by replacing expensive third-party solutions with an open-source, omnichannel CRM.

## Current Position
- **Phase:** 01 of 07 (Foundation (Core Infrastructure))
- **Plan:** 04 of 04 in current phase
- **Status:** Complete
- **Progress:** ████████████████████ 100%

## Performance Metrics
- **Estimated launch:** Q2 2025
- **Projected cost savings:** 50-100k ₽/month
- **Target user base:** 150,000 active customers

## Accumulated Context
### Decisions Made
- Project architecture: Microservices-based with API gateway
- Primary channels: Telegram, VK, mobile app
- Database: PostgreSQL with Redis cache
- ERP integration: ERPNext
- **Test infrastructure:** SQLAlchemy URLs vs absolute filesystem paths for SQLite - critical distinction for pytest fixtures

### Blockers
- None

## Session Continuity
Last updated: 2026-03-02
Next step: Plan Phase 01 - Foundation (Core Infrastructure)
